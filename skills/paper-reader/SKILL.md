...
5. **图片只用外链**: 所有图片使用 arXiv HTML / 项目主页的原始 URL，不下载到本地，避免 git history 中出现图片文件

> 公式/图片/表格的详细质量规范见 `references/quality-standards.md`

### 图片获取流程（只用外链 URL）

**目标**: 确保笔记中包含论文的**所有 Figure**，先统计论文 Figure 总数再逐一获取。**所有图片使用外链 URL，不下载到本地**。
...
4. 笔记中用 `![Figure X](url)` 外链嵌入
5. **URL 去重**：写入前检查 URL 中是否有重复的 arxiv_id 路径段（如 `2603.05312v1/2603.05312v1/`），有则删除重复段。详见 `references/image-troubleshooting.md`

> ar5iv 编号不一定对应 Figure 编号，排错见 `references/image-troubleshooting.md`

> **重要**: 不下载图片到本地，不创建 `assets/` 目录。图片始终使用在线外链 URL，`image_source` 始终为 `online`。
...
- **`references/image-troubleshooting.md`** — ar5iv 图片编号对应、URL 去重检查
